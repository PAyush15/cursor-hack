...
/**
 * Render uploaded models in the existing models grid (at the beginning)
 */
function renderUploadedModels() {
    ...
    // Add uploaded models at the beginning of the grid
    const modelsToShow = uploadedModels.slice(0, 3); // Show up to 3 uploaded models

    modelsToShow.reverse().forEach((model, index) => {
        // Check if this model card already exists
        if (document.getElementById(`uploaded-${model.id}`)) return;

        const card = document.createElement('a');
        card.id = `uploaded-${model.id}`;
        card.href = `viewer.html?model=uploaded&id=${model.id}`;
        card.className = 'model-card glass-card';

        const isLatest = index === modelsToShow.length - 1; // Last in reversed = first in original = latest
        ...
        card.innerHTML = `
            <div class="model-preview" style="display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); min-height: 120px; position: relative;">
                ...
                ${isLatest ? '<span style="position: absolute; top: 8px; right: 8px; background: white; color: #22c55e; padding: 2px 8px; border-radius: 4px; font-size: 0.7rem; font-weight: 600;">NEW</span>' : ''}
            </div>
